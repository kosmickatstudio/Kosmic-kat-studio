 ...
 * Phase 3: CHAT intent -> existing V3 execution -> conversational output.
...
(function installKosmicVideoChatGeneration(){
  ...
  function currentMeta(){
    ...
    const route=v.route||"seedance-2.5-text-to-video";
    const item=window.KOSMIC_EVOLINK_VIDEO?.index?.[route];
  ...
  function waitForV3(baseline){
    return new Promise((resolve,reject)=>{
      ...
      const tick=()=>{
        ...
        if(!v.busy && Array.isArray(v.history) && v.history.length>baseline){
          const item=v.history[0];
          if(item?.error)reject(new Error(item.error));
          else if(item?.url)resolve(item);
          else reject(new Error("Video generation finished without a result URL."));
          return;
        }
        if(Date.now()-started>900000){reject(new Error("Video generation timed out."));return;}
        setTimeout(tick,350);
      };
      tick();
    });
  ...
  async function run(prompt){
    ...
    const button=document.getElementById("kkv3Generate");
    ...
    try{
      ...
      q.setActiveGeneration(Object.assign({},output,{status:"completed"}));
      render();
      return output;
    }catch(err){
      q.messages=q.messages.filter(m=>m.id!==pending.id);
  ...
  function submit(){
    ...
    q.composerDraft="";input.value="";input.style.height="auto";q.syncLegacy();render();
    run(text||"Use the attached references as the visual basis for the next video.").catch(()=>{});
  }

  function useOutput(id){
    const q=st(),o=(q.outputs||[]).find(x=>x.id===id);if(!o)return;
    q.addReference({kind:"video",url:o.resultUrl,name:"Generated video"});q.syncLegacy();
    const input=document.getElementById("kkvcInput");if(input)input.focus();
  }
  ...
  function intercept(e){
    const target=e.target?.closest?.("#kkvcSend");if(!target)return;
    e.preventDefault();e.stopImmediatePropagation();submit();
  }
