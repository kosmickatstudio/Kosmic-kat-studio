...
function renderHome(el){
  ...
  el.innerHTML=`
    ...
    <div style="display:flex;gap:10px;overflow-x:auto;padding:2px 2px 10px;margin-bottom:${recentVideos.length?'4':'14'}px">
      ${recentImages.map(a=>`<div data-meta="${jsonAttr({prompt:a.prompt||"",assetId:a.id,mediaUrl:a.url,mediaType:"image",model:a.model||"",providerLabel:a.providerLabel||"Image",resolution:a.resolution||"",aspectRatio:a.aspectRatio||""})}" onclick="openGenerationInfoModalFromCard(this)" style="flex-shrink:0;width:140px;border-radius:16px;overflow:hidden;background:var(--surface);border:1px solid var(--border);box-shadow:0 3px 14px rgba(61,31,122,0.08);cursor:pointer;position:relative">
        ${a.reviewStatus?`<div style="position:absolute;top:6px;right:6px;width:18px;height:18px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;color:#fff;background:${a.reviewStatus==='approved'?'var(--green)':'var(--red)'};box-shadow:0 1px 4px rgba(0,0,0,0.3)">${a.reviewStatus==='approved'?'✓':'✕'}</div>`:''}
    ...
    <div style="display:flex;gap:10px;overflow-x:auto;padding:2px 2px 14px">
      ${recentVideos.map(a=>`<div data-meta="${jsonAttr({prompt:a.prompt||"",assetId:a.id,mediaUrl:a.url,mediaType:"video",model:a.model||"",providerLabel:a.providerLabel||"Video",resolution:a.resolution||"",aspectRatio:a.aspectRatio||"",duration:a.duration||""})}" onclick="openGenerationInfoModalFromCard(this)" style="flex-shrink:0;width:200px;border-radius:16px;overflow:hidden;background:var(--surface);border:1px solid var(--border);box-shadow:0 3px 14px rgba(61,31,122,0.08);cursor:pointer;position:relative">
        ${a.reviewStatus?`<div style="position:absolute;top:6px;right:6px;width:18px;height:18px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;color:#fff;background:${a.reviewStatus==='approved'?'var(--green)':'var(--red)'};box-shadow:0 1px 4px rgba(0,0,0,0.3)">${a.reviewStatus==='approved'?'✓':'✕'}</div>`:''}
