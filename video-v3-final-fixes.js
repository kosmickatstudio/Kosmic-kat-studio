/* KOSMIC KAT — Video V3 final interaction/layout guard
 * This is a compatibility layer for the migrated Video UI. It does not own
 * provider routes, credentials, or generation.
 */
(function installKosmicVideoV3FinalFixes(){
  "use strict";
  if(window.__kosmicVideoV3FinalFixes)return;
  window.__kosmicVideoV3FinalFixes=true;
  ...
  function css(){
    if($("kk-video-v3-final-css"))return;
    const s=document.createElement("style");s.id="kk-video-v3-final-css";
    s.textContent=`
      ...
      .kkv3-final-model-empty{padding:14px;text-align:center;font-size:10px;color:var(--texts,#9488ae);}

      /* Duration is kept as the real range input so keyboard/touch controls
         remain accessible. The visual state is painted from its value. */
  ...
  function syncDuration(){
    ...
    r.value=String(value);n.value=String(value);
    r.style.setProperty("--kk-duration-fill",(max>min?((value-min)/(max-min))*100:0).toFixed(1)+"%");
    const label=r.closest(".kkv3-field")?.querySelector("label b");if(label)label.textContent=value+"s";
    let row=r.parentElement;
    if(row&&!row.classList.contains("kkv3-duration-wrap-final")){
      row.classList.add("kkv3-duration-wrap-final");
      let meta=row.parentElement?.querySelector(".kkv3-duration-meta-final");
      if(!meta){meta=document.createElement("div");meta.className="kkv3-duration-meta-final";meta.innerHTML=`<span>Min ${min}s</span><span>Max ${max}s</span>`;row.parentElement?.appendChild(meta);}
    }
    if(!r.__kkFinalDuration){
      r.__kkFinalDuration=true;
      r.addEventListener("input",()=>{
        const v=Math.max(min,Math.min(max,Number(r.value||min)));
        r.value=String(v);n.value=String(v);r.style.setProperty("--kk-duration-fill",(max>min?((v-min)/(max-min))*100:0).toFixed(1)+"%");
        const label=r.closest(".kkv3-field")?.querySelector("label b");if(label)label.textContent=v+"s";
        const st=window.__kosmicVideoV3State;if(st)st.duration=v;
      });
      n.addEventListener("input",()=>{
        const v=Math.max(min,Math.min(max,Number(n.value||min)));n.value=String(v);r.value=String(v);r.dispatchEvent(new Event("input",{bubbles:true}));
      });
      n.addEventListener("change",()=>{const st=window.__kosmicVideoV3State;if(st)st.duration=Number(n.value||min);r.dispatchEvent(new Event("change",{bubbles:true}));});
    }
  }

  function closeMenu(){
    const m=$("kkv3FinalModelMenu"),t=$("kkv3FinalModelTrigger");m?.classList.remove("open");t?.setAttribute("aria-expanded","false");
  }
  function positionMenu(){
    const t=$("kkv3FinalModelTrigger"),m=$("kkv3FinalModelMenu");if(!t||!m)return;
    const r=t.getBoundingClientRect();const mw=Math.min(360,window.innerWidth-20);let left=Math.max(10,Math.min(r.left,window.innerWidth-mw-10));let top=r.bottom+7;const mh=Math.min(window.innerHeight*.52,360);if(top+mh>window.innerHeight-10)top=Math.max(10,r.top-mh-7);m.style.width=mw+"px";m.style.left=left+"px";m.style.top=top+"px";m.style.maxHeight=mh+"px";
  }
  function syncModel(){
  ...
  function refresh(){
    cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{if(!active()){closeMenu();return;}css();scrubLegacy();syncModel();syncDuration();ensureGallery();});
  }

  function observe(){
    const r=root();if(!r||observer)return;
    observer=new MutationObserver(()=>{observer.disconnect();refresh();requestAnimationFrame(()=>{if(root())observer.observe(root(),{childList:true,subtree:true});});});
    observer.observe(r,{childList:true,subtree:true});
  }

  function boot(){
    css();refresh();observe();
    if(!outsideBound){outsideBound=true;document.addEventListener("click",e=>{if(!e.target?.closest?.(".kkv3-final-model-picker")&&!e.target?.closest?.(".kkv3-final-model-menu"))closeMenu();},true);}
  }
