...
(function installEvoLinkVideoCanvasAdapter(){
  ...
  const routeIds=()=>{
    ...
    for(const card of api.catalog){
      for(const route of card.routes||[]){
        if(!specialized.has(route.id))out.push({card,route});
      }
    }
    return out;
  };
  ...
  function installMaps(){
    ...
    const seen=new Set(wanted.map(x=>x.route.id));
    ...
    wanted.forEach(({card,route})=>{
      ...
      grouped[card.group].push({value:route.id,name:`${card.name} · ${route.label||route.mode}`,sub:`${card.provider} · EvoLink`});
    });
    const evoGroups=Object.entries(grouped).map(([group,items])=>({label:`EvoLink · ${group}`,items}));
    const oldEvo=VIDEO_MODEL_GROUPS.filter(g=>String(g.label||"").startsWith("EvoLink · "));
    oldEvo.forEach(g=>{const i=VIDEO_MODEL_GROUPS.indexOf(g);if(i>=0)VIDEO_MODEL_GROUPS.splice(i,1);});
    ...
    for(const {card,route} of wanted){
      ...
      if(typeof VIDEO_MODEL_DURATIONS!=="undefined"&&schema.duration){
        ...
        if(Number.isFinite(lo)&&Number.isFinite(hi)&&lo<=hi){
          const options=[];for(let n=lo;n<=Math.min(60,hi);n++)options.push(n);
          VIDEO_MODEL_DURATIONS[route.id]={options,format:v=>Number(v)};
        }
      }
      ...
      if(typeof VIDEO_MODEL_CAPABILITIES!=="undefined"){
        ...
        const cap=Object.assign({},VIDEO_MODEL_CAPABILITIES[route.id]||{}, {
          provider:"evolink",evolink:true,route:route.id,mode:route.mode,schema,
          maxImages:refs.images||0,maxVideos:refs.videos||0,maxAudios:refs.audios||0,
          multiImage:route.mode!=="text"&&route.mode!=="edit"&&route.mode!=="extend",
  ...
  function syncCanvasUi(){
    ...
    const html=modelOptionsHTML("video",null,seenLive(current)?current:"seedance-2.5-text-to-video");
  ...
  function seenLive(id){return !!window.KOSMIC_EVOLINK_VIDEO?.index?.[id]&&!specialized.has(id);}
  ...
  async function resolveRefsForEvoLink(){
    const s=state();
    const images=extractHosted(s.vcMultiImages),videos=extractHosted(s.vcMultiVideos),audios=extractHosted(s.vcMultiAudios);
    const all=[...images,...videos,...audios];if(!all.length)return{images:[],videos:[],audios:[]};
    const directOk=u=>/^https?:\/\//i.test(u);
    if(all.every(directOk))return{images, videos, audios};
    ...
    const falKey=String(typeof gs==="function"?gs("api_falai",""):"").trim();
    if(!falKey)throw new Error("Reference uploads need a hosted URL. Add the existing fal.ai storage key in Settings or use hosted reference URLs.");
    const hosted=await uploadRefsToFal(all.map(u=>({dataUrl:u,name:""})),falKey);
  ...
  async function generateFromExistingCanvas(){
    ...
    const send=document.getElementById("vcSendBtn");
    if(send)send.dataset.vcSending="1";
  ...
  function wrapSend(){
    ...
    wrapper.__kosmicEvoCanvasWrapped=true;wrapper.__kosmicEvoOriginal=fn;
    window.sendVcChatPrompt=wrapper;return true;
  }

  function boot(){
    if(!window.KOSMIC_EVOLINK_VIDEO?.index||typeof VIDEO_MODEL_GROUPS==="undefined")return false;
    installMaps();syncCanvasUi();return wrapSend();
  }
  if(!boot()){
    let tries=0;const timer=setInterval(()=>{if(boot()||++tries>160)clearInterval(timer);},50);
  }
  setInterval(()=>{if(document.getElementById("vcModel")){syncCanvasUi();wrapSend();}},800);
})();
