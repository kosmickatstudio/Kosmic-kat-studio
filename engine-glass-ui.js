...
(function(){
  ...
  let observer=null,wireQueued=false,visionRetryTimer=null;
  const css=`
#dcInputBar.${ROOT}{position:relative!important;margin:10px!important;padding:12px!important;border:1px solid rgba(255,255,255,.78)!important;border-top-color:rgba(255,255,255,.94)!important;border-radius:28px!important;background:linear-gradient(135deg,rgba(255,255,255,.76),rgba(246,242,255,.58))!important;box-shadow:0 18px 50px rgba(86,61,140,.13),inset 0 1px 0 rgba(255,255,255,.88)!important;overflow:visible!important;contain:paint!important}
...
.kg-settings-panel{position:relative;z-index:5;display:none;margin:0 0 9px;padding:10px;border:1px solid var(--glass-brd);border-radius:20px;background:rgba(255,255,255,.92);box-shadow:0 14px 34px rgba(61,31,122,.10),inset 0 1px 0 rgba(255,255,255,.70)}.kg-settings-panel.open{display:block}.kg-settings-label{font-size:9px;font-weight:800;letter-spacing:.12em;color:var(--texts);margin:1px 2px 7px;text-transform:uppercase}
#dcEngineModes.kg-modes{display:flex!important;gap:7px!important;margin:0 0 9px!important;padding:1px!important;overflow-x:auto!important;scrollbar-width:none!important}#dcEngineModes.kg-modes::-webkit-scrollbar{display:none}#dcEngineModes.kg-modes button{min-height:38px!important;padding:8px 12px!important;border-radius:13px!important;border:1px solid var(--glass-brd)!important;background:rgba(255,255,255,.72)!important;color:var(--textm)!important;font-size:10px!important;font-weight:800!important;white-space:nowrap!important}
#dcEngineModelRow.kg-model-row{margin:0!important;padding:8px 9px!important;border:1px solid var(--glass-brd)!important;border-radius:13px!important;background:rgba(255,255,255,.72)!important}#dcEngineModelRow.kg-model-row select{height:34px!important;border-radius:10px!important;background:var(--surface)!important;color:var(--text)!important}
...
#dcInputBar.${ROOT} textarea#dcInput{min-height:42px!important;max-height:150px!important;background:rgba(255,255,255,.76)!important;border:1px solid rgba(61,31,122,.10)!important;border-radius:18px!important;padding:11px 14px!important;resize:none!important;color:var(--text)!important}#dcInputBar.${ROOT} textarea#dcInput:focus{border-color:var(--ice)!important;box-shadow:0 0 0 3px var(--glow-ice)!important;outline:none!important}#dcInputBar.${ROOT} .kg-attach-btn{width:42px!important;height:42px!important;border-radius:14px!important;border:1px solid var(--glass-brd)!important;background:rgba(255,255,255,.70)!important;color:var(--violet)!important;font-size:20px!important}#dcInputBar.${ROOT} .ig-send-btn{width:44px!important;height:44px!important;border-radius:15px!important;border:0!important;background:linear-gradient(135deg,var(--violet),var(--ice))!important;color:#fff!important;box-shadow:0 8px 22px var(--glow-ice)!important}
...
`;
  ...
  function queueWire(){if(wireQueued)return;wireQueued=true;queueMicrotask(()=>{wireQueued=false;if(!observer){wire();return}observer.disconnect();try{wire()}finally{observer.observe(document.body,{childList:true,subtree:true})}})}
  function nodeTouchesEngine(node){
    if(!node||node.nodeType!==1)return false;
    if(node.id==="dcInputBar"||node.id==="dcEngineModes"||node.id==="dcEngineModelRow"||node.id==="dcRefStrip"||node.id==="kgSettingsPanel"||node.id==="kgGlassTop")return true;
    return !!node.querySelector?.("#dcInputBar,#dcEngineModes,#dcEngineModelRow,#dcRefStrip,#kgSettingsPanel,#kgGlassTop");
  }
  function mutationsNeedWire(records){return records.some(r=>Array.from(r.addedNodes||[]).some(nodeTouchesEngine)||Array.from(r.removedNodes||[]).some(nodeTouchesEngine))}
  function installVisionBridge(){
    const original=window.callAiVision;if(typeof original!=="function"||original.__kosmicEngineVisionRouter)return true;
    const routed=async function(refs,prompt,system){
      const d=window.S&&S.directorChat?S.directorChat:null,draft=d&&d.draft?d.draft:null,provider=draft&&draft.brainModel,model=draft&&d.engineModel;
    ...
    routed.__kosmicEngineVisionRouter=true;window.callAiVision=routed;return true;
  }
  function observe(){
    wire();
    observer=new MutationObserver(records=>{if(mutationsNeedWire(records))queueWire();});
    observer.observe(document.body,{childList:true,subtree:true});
    installVisionBridge();
    [250,1000,2500,5000].forEach(delay=>setTimeout(()=>{wire();installVisionBridge()},delay));
    clearTimeout(visionRetryTimer);
    visionRetryTimer=setTimeout(()=>{if(!installVisionBridge())visionRetryTimer=setTimeout(()=>installVisionBridge(),5000)},10000);
  }
