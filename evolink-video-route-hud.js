...
(function installEvoLinkRouteHud(){
  ...
  const boot=()=>{
    ...
    const pricing=window.KOSMIC_EVOLINK_VIDEO_PRICING||{};
    ...
    const update=()=>{
      ...
      const p=pricing[route.id]||pricing[route.model?.id];
      const price=p?`from $${p.from<0.01?p.from.toFixed(4):p.from.toFixed(3)} / ${p.unit}`:"Provider rate";
      hud.innerHTML=`<div class="evo-route-hud-top"><span class="evo-route-hud-kicker">EVOLINK ROUTE</span><span class="evo-route-hud-name">${esc(modeLabel)}</span></div><div class="evo-route-hud-id">${esc(route.id)}</div><div class="evo-route-hud-meta">${esc(details.join("  ·  ")||"Schema available in provider API")}</div><div class="evo-route-hud-price">${esc(price)}</div>`;
    };
