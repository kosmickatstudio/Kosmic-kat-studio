...
(function installEvoLinkAdapterFallback(){
  ...
  const boot=()=>{
    ...
    window.generateEvoLinkVideo=async function(model,prompt,options={}){
      ...
      ["aspect_ratio","duration","quality","output_format","generate_audio","web_search","image_urls","video_urls","audio_urls","callback_url","mask_url","model_params"].forEach(k=>{
        const v=options[k];if(v!==undefined&&v!==null&&v!==""&&(!Array.isArray(v)||v.length))body[k]=v;
      });
      if(body.duration==="auto"||body.duration===-1)body.duration="auto";
