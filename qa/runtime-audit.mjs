...
async function main() {
  ...
  try {
    ...
    fail('Initial navigation threw', String(e?.message || e));
  }
  ...
  try {
    openedSettings = await page.evaluate(() => {
      try {
  ...
  if (settingsState.found) {
    ...
    if (settingsState.parent !== 'BODY') warn('Settings sheet is not mounted directly under BODY', settingsState.parent);
    ...
    if (settingsState.sheet.visibility !== 'visible' || settingsState.sheet.opacity === '0' || settingsState.sheet.display === 'none') fail('Settings sheet exists but is not visibly open', JSON.stringify(settingsState.sheet));
  ...
  const report = {
    ...
    title, failures, warnings, consoleErrors, pageErrors, requestFailures, badResponses, modelSnapshot, settingsState, mobile,
  };
